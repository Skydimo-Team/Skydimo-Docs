...
引导设置只询问尚无答案的设备，因此这个字段决定了同一台设备不会每次启动都被再问一遍。跳过也算答案：用户被问过并且拒绝了。设回 `false` 即可让该设备重新被询问，换了灯带的用户需要的正是这个。

它与"设备是否已有配置"是两回事。Core 一发现硬件就会写设备配置，用来记住身份与发现路径，所以配置文件存在只说明见过这台设备。配置文件早于此字段的设备按已确认处理。

| 字段 | 类型 | 必填 | 说明 |
|------|------|:----:|------|
| `port` | string | 是 | 设备端口标识 |
| `confirmed` | boolean | 是 | 用户是否已被询问过该设备 |
