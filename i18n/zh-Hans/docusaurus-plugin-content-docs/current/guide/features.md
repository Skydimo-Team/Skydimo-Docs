...
- **内置效果** —— 包含单色、矩阵测试等基础效果
- **灯效插件** —— 安装社区效果，或使用 Lua / native-c 编写自己的效果
- **参数化** —— 每个灯效可提供滑块、范围滑块、下拉选择、开关、颜色和多色调色板
...
| 运行时 | 适用场景 |
|--------|----------|
| `lua` | 快速迭代、硬件协议胶水、扩展逻辑 |
| `native-c` | 高性能灯效、原生集成、通过稳定 C ABI 构建语言绑定 |

## WebSocket API

- **JSON-RPC 2.0** —— 标准协议，支持完整的程序化控制
- **事件推流** —— 实时设备状态变化和 LED 颜色更新
- **仅本机传输** —— 从 `3.0.0-dev.3` 开始，Core 监听 `127.0.0.1`，且不再需要单独的鉴权握手
