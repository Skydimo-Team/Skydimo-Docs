...
- 通过插件管理 UI 或 API 导入 `.skyplugin`/`.zip` 包
- 选择扫描到的插件 ID 进行安装
- 在插件列表中验证结果

本地开发时，将插件目录放入 `<config_dir>/plugins/dev/`，修改后执行“刷新插件”。

优点：

- 安装/更新行为更可预期
- 插件代码与插件数据分离更清晰
- 可追踪安装来源（`bundled`、`import-dev`、`package`）
...
|------|------|
| `bundled` | 应用打包插件 |
| `import-dev` | 直接从 `<config_dir>/plugins/dev/` 加载的开发插件 |
| `package` | 包导入安装流程 |
...
- 删除当前已安装插件副本。
- 可选择是否同时删除插件数据。
- 开发插件从 `<config_dir>/plugins/dev/` 直接加载，不会被 `delete_plugin` 删除。
