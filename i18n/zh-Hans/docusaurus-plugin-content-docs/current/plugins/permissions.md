...
插件通过 `manifest.json` 声明所需的宿主能力。Core 会检查下文列出的受控 API，但这是一层能力边界，并不是完整的进程沙箱。部分管理 API 有意不受额外权限控制，而 native-C 插件会作为受信任的进程内代码执行。
...
- 扩展插件的管理类 API（例如插件安装/删除/刷新、快捷键、语言、捕获设置）目前不需要额外 manifest 权限。若扩展将这些能力暴露给用户或外部工具，应将其视为可信管理扩展。
