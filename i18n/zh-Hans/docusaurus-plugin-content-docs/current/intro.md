...
**Skydimo** 是一款基于 Rust 和 React 构建的跨平台 RGB 灯光控制应用程序，提供强大的 **Lua 插件系统**，用于扩展设备支持并创建自定义灯光效果。
...
- **Core** 是独立的 Rust 可执行文件，负责设备管理、灯效引擎、插件运行时和 WebSocket 服务器。
- **UI** 是通过 WebSocket JSON-RPC 2.0 与 Core 通信的 React 前端，可在 Tauri 内或标准浏览器中运行。
